...
# ==================== 全局日志开关 ====================
# 可以通过环境变量或代码设置
# 环境变量: PENSHOT_LOG_ENABLED=0 或 PENSHOT_LOG_ENABLED=false 来禁用日志
# 环境变量: PENSHOT_LOG_LEVEL=ERROR 来设置最低日志级别

class LoggingGlobalConfig:
    """全局日志配置"""
    _instance = None
    _log_enabled = True
    ...
    def _initialize(self):
        """初始化配置"""
        # 从环境变量读取日志开关
        env_enabled = os.getenv('PENSHOT_LOG_ENABLED', '').lower()
        if env_enabled in ('0', 'false', 'no', 'off'):
            self._log_enabled = False
            print("[PenShot] 日志输出已禁用 (PENSHOT_LOG_ENABLED=0)")

        # 从环境变量读取日志级别
        env_level = os.getenv('PENSHOT_LOG_LEVEL', '').upper()
        if env_level in self._level_map:
            self._min_level = self._level_map[env_level]
            print(f"[PenShot] 日志级别设置为: {env_level}")
    ...
    def set_log_enabled(self, enabled: bool):
        ...
        if enabled:
            print("[PenShot] 日志输出已启用")
        else:
            print("[PenShot] 日志输出已禁用")
    ...
    def set_min_level_by_name(self, level_name: str):
        """通过名称设置最低日志级别"""
        level = self._level_map.get(level_name.upper())
        if level is not None:
            self._min_level = level


# 创建全局配置实例
_global_log_config = LoggingGlobalConfig()
...
def set_log_enabled(enabled: bool):
    ...
    """
    _global_log_config.set_log_enabled(enabled)
...
def set_log_level(level: str):
    ...
    """
    _global_log_config.set_min_level_by_name(level)


def is_log_enabled() -> bool:
    """检查日志是否启用"""
    return _global_log_config.is_log_enabled()


# ==================== 日志过滤器 ====================

class GlobalLogFilter(logging.Filter):
    """全局日志过滤器"""

    def filter(self, record):
        # 检查全局开关
        if not _global_log_config.is_log_enabled():
            return False

        # 检查日志级别
        if record.levelno < _global_log_config.get_min_level():
            return False
...
class DailyRotatingFileHandler(logging.Handler):
    ...
    def __init__(self, log_dir: str, name: str = 'PenShot', max_bytes: int = 10 * 1024 * 1024,
        ...
        # 添加全局过滤器
        self.addFilter(GlobalLogFilter())
...
class LoggingConfigManager:
    ...
    def load_config(self) -> Dict[str, Any]:
        ...
        if not self.logging_config_file.exists():
            if _global_log_config.is_log_enabled():
    ...
    def to_logging_dict(self) -> Dict[str, Any]:
        ...
        if "console" in config:
            ...
            logging_dict["handlers"]["console"]["filters"] = ["global_filter"]

        if "file" in config:
            logging_dict["handlers"]["file"] = self.get_file_handler_config()
            logging_dict["handlers"]["file"]["filters"] = ["global_filter"]
...
class Logger:
    ...
    def _create_console_handler(self, handler_config: Dict[str, Any], fmt: str,
        ...
        # 添加全局过滤器
        console_handler.addFilter(GlobalLogFilter())
    ...
    def _create_file_handler(self, handler_config: Dict[str, Any], fmt: str,
        ...
        # 添加全局过滤器
        file_handler.addFilter(GlobalLogFilter())
    ...
    def _check_windows_color(self):
        """检查Windows颜色支持"""
        if IS_WINDOWS and not HAS_COLORAMA and _global_log_config.is_log_enabled():
            self.warning("在Windows平台上运行，但未安装colorama库，可能无法显示彩色日志。建议安装: pip install colorama")

    # ==================== 日志方法 ====================
    # 所有日志方法都检查全局开关，提供双重保障
